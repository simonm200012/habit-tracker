...
export function NotificationPrefsForm({
  ...
  return (
    ...
    >
      <Row
        label="Morning brief"
        description="Today's habits, top streaks, insights."
        timeName="morning_brief_hour"
        timeDefault={prefs.morning_brief_hour ?? 7}
      ...
      <Row
        ...
        description="Heads-up when active streaks are about to break."
        timeName="evening_alert_hour"
        timeDefault={prefs.evening_alert_hour ?? 20}
      ...
      />
      <Row
        label="Weekly review (Sundays 6pm)"
        description="The week in numbers + a plan-for-next-week prompt."
      ...
      <div>
        ...
        <input
          name="timezone"
          defaultValue={prefs.timezone ?? detectedTz}
          placeholder={detectedTz}
          className="w-full sm:w-72 px-3 py-2 rounded-lg border border-slate-200 bg-white text-slate-900 font-mono focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition"
        />
        <p className="text-[10px] text-slate-500 mt-1">
          Detected: <code className="text-slate-700">{detectedTz}</code>. We send each notification at the local hour you pick.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
        ...
        {timeName && (
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">at</span>
            <input
              name={timeName}
              type="number"
              min={0}
              ...
              className="w-14 px-2 py-1 rounded-md border border-slate-200 bg-white text-slate-900 font-semibold text-sm tabular-nums text-center focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition"
            />
            <span className="text-xs text-slate-500 font-medium">:00 local</span>
          </div>
        )}
      </div>
